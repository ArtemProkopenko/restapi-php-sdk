...
class Immocaster_Immobilienscout_Rest extends Immocaster_Immobilienscout
{
     ...
     * @return mixed
     */
    private function doRequest($sPath, $aArgs, $aRequired = [], $sFunctionName, $oToken = null, $postRequest = false)
    {
    ...
    {
        $aRequired = ['geocoordinates', 'realestatetype'];
        $oToken = null;
        $sSecret = null;
        if (isset($aArgs['username'])) {
            list($oToken, $sSecret) = $this->getApplicationTokenAndSecret($aArgs['username']);
    ...
    {
        $aRequired = ['geocodes', 'realestatetype'];
        $oToken = null;
        $sSecret = null;
        if (isset($aArgs['username'])) {
            list($oToken, $sSecret) = $this->getApplicationTokenAndSecret($aArgs['username']);
    ...
    {
        $aRequired = ['exposeid', 'request_body'];
        $oToken = null;
        $sSecret = null;
        if (isset($aArgs['username'])) {
            list($oToken, $sSecret) = $this->getApplicationTokenAndSecret($aArgs['username']);
    ...
    {
        $aRequired = ['contactid', 'username', 'contact'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
        if (isset($aArgs['contact'])) {
    ...
    {
        $aRequired = ['username', 'service', 'estate'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
        if (isset($aArgs['service']) && isset($aArgs['estate'])) {
            if (isset($aArgs['estate']['xml'])) {
                ...
                if (!$oXml->setFormat(strtolower($aArgs['service']), ['estate_type' => $aArgs['estate']['type']])) {
    ...
    {
        $aRequired = ['username', 'attachmentid', 'estateid'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
        $oToken = null;
    ...
    {
        $aRequired = ['exposeid', 'username', 'service', 'estate'];
    ...
    {
        $aRequired = ['username', 'exposeid', 'channelid'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
        if (isset($aArgs['xml'])) {
    ...
    {
        $aRequired = ['username', 'ontopplacementtype'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
        if (isset($aArgs['body'])) {
    ...
    {
        $aRequired = ['username', 'realestateid', 'ontopplacementtype'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
    ...
    {
        $aRequired = ['username', 'realestateids', 'ontopplacementtype'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
    ...
    {
        try {
            parent::requiredArgs($aArgs, ['verifyApplication'], ' ' . __FUNCTION__);
    ...
    {
        try {
            if (parent::requiredArgs($aArgs, ['callback_url'], ' ' . __FUNCTION__)) {
                ...
                if ($this->_bAuthenticateWithoutDB === false) {
                    Immocaster_Data_Mysql::getInstance()
                        ->saveRequestToken($aResult['oauth_token'], $aResult['oauth_token_secret']);
    ...
    private function registerAccess($aArgs)
    {
        try {
            ...
            $req = parent::restRequest('oauth/access_token', [], true);
            ...
            $opts = ['http' => ['header' => $authHeader . ',oauth_signature_method="HMAC-SHA1",oauth_signature="' .
    ...
    {
        $aRequired = ['username', 'estateid', 'videoid'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
        if (!isset($aArgs['title'])) {
...
xmlns:ns3="http://rest.immobilienscout24.de/schema/platform/gis/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">' . $sBreak;
    ...
    {
        $aRequired = ['username', 'estateid', 'attachmentid', 'type'];
        if (!isset($aArgs['username'])) {
            $aArgs['username'] = $this->_sDefaultUsername;
        }
